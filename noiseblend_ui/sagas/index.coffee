...
import { fetchAttributesUsage, openWebsocket, send, sendWs } from './stats'
import {
    checkout,
    fetchDislikes,
    removeDislike
...
export default getRootSaga = (ctx) ->
    ...
    return () ->
        yield all([
            ...
            takeLatest(StatsTypes.SEND, send, api)
            takeLatest(StatsTypes.FETCH_ATTRIBUTES_USAGE, fetchAttributesUsage, api)
            takeLatest(StatsTypes.OPEN_WEBSOCKET, openWebsocket)
            takeLatest(
            ...
            takeLatest(UserTypes.CHECKOUT, checkout, api)
        ])
