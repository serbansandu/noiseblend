...
from . import APP_NAME, app, config, logger, spf
from .constants import APP_USER_FLAGS, SIDEBAR_ATTRIBUTE_METRIC
...
async def oauth_code(request):
    ...
    async with spotify.async_db_session(readonly=False) as conn:
        code = str(uuid4())
        await conn.execute(
...
async def oauth_token(request):
    ...
    async with spotify.async_db_session(readonly=False) as conn:
        ...
        insert_token_stmt = await conn.prepare(SQL.insert_token)
        token = await insert_token_stmt.fetchrow(user["id"])
        resp = {
...
async def blend_token(request):
    ...
    async with spotify.async_db_session(readonly=True) as conn:
        blend_token_stmt = await conn.prepare(SQL.blend_token)
        token = await blend_token_stmt.fetchval(spotify.user_id, blend)

    if not token:
        async with spotify.async_db_session() as conn:
            upsert_blend_stmt = await conn.prepare(SQL.upsert_blend)
            token = await upsert_blend_stmt.fetchval(spotify.user_id, blend, gentoken())
...
async def authenticate(request):
    ...
    async with spotify.async_db_session() as conn:
        app_user_stmt = await conn.prepare(SQL.upsert_app_user)
        app_user = await app_user_stmt.fetchrow(user.id, bool(user.email))
        if not app_user:
...
async def logout(request):
    ...
    async with spotify.async_db_session() as conn:
        ...
        await conn.execute(
            "UPDATE app_users SET auth_token = $2 WHERE id = $1",
            spotify.user_id,
            uuid4(),
        )
...
async def me(request):
    spotify = request["spotify"]
    return with_cache(await get_user_dict(spotify))
...
async def fetch_dislikes(request):
    ...
    async with spotify.async_db_session(readonly=True) as conn:
        dislikes = {}
        if _type == "all":
            dislikes_stmt = await conn.prepare(SPFY_SQL.user_dislikes)
            dislike_rows = [d[0] for d in await dislikes_stmt.fetch(spotify.user_id)]
        ...
        if dislikes.get("genres"):
            ...
            dislikes["genres"] = [
                {"name": genre, "image": images.get(genre)}
                for genre in dislikes["genres"]
            ]
        ...
        if dislikes.get("countries"):
            ...
            dislikes["countries"] = [
                {
                    "code": c,
                    "name": Country.get_iso_country(c).name,
                    "image": images.get(c),
                }
...
async def change_user_details(request):
    ...
    shared_on_social = request.json.get("shared_on_social")
    ...
    async with spotify.async_db_session() as conn:
        ...
        # if email and spotify.user.email != email:
        #     spotify.user.email = email
        #     spotify.app_user.email_confirmed = False
        #     spotify.app_user.email_token = gentoken()
        #     if not spotify.app_user.email_confirmed:
        #         send_confirmation_email(
        #             email,
        #             spotify.user.display_name or spotify.username,
        #             spotify.app_user.email_token,
        #         )
        if shared_on_social:
            shared_at = await conn.fetchval(
                "SELECT shared_at FROM app_users WHERE id = $1", spotify.user_id
            )
            if not shared_at:
                app_user_fields["shared_at"] = datetime.utcnow()
...
async def top_artists(request):
    ...
    if not _all:
        if not time_range:
            async with spotify.async_db_session(readonly=True) as conn:
                artist_time_range_stmt = await conn.prepare(SQL.user_artist_time_range)
                time_range = await artist_time_range_stmt.fetchval(spotify.user_id)
...
async def top_genres(request):
    ...
    async def get_genres(playlists):
        ...
        try:
            await assign_images(
                spotify, genres, "name", "genre", image_width, image_height
            )
    ...
    async with spotify.async_db_session(readonly=True) as conn:
        ...
        if not _all:
            if not time_range:
                genre_time_range_stmt = await conn.prepare(SQL.user_genre_time_range)
                time_range = await genre_time_range_stmt.fetchval(spotify.user_id)
        ...
        playlists_stmt, *genres = await asyncio.gather(
            conn.prepare(query), *genre_requests
        )
        if not any(genres):
            return [] if not _all else {}

        playlists = [
            await playlists_stmt.fetch(g, image_width, image_height) for g in genres
        ]
...
async def fetch_countries(request):
    ...
    async with spotify.async_db_session(readonly=True) as conn:
        ...
        playlists_stmt = await conn.prepare(query)
        playlists = await playlists_stmt.fetch(
            ignore, image_width, image_height, spotify.user_id
        )
    ...
    try:
        await assign_images(
            spotify, countries, "code", "country", image_width, image_height
        )
    except Exception as exc:
        logger.error("Error fetching images:")
        logger.exception(exc)
...
async def fetch_cities(request):
    ...
    async with spotify.async_db_session(readonly=True) as conn:
        if not country:
            user_country_stmt = await conn.prepare(SQL.user_country)
            user_country, preferred_country = await user_country_stmt.fetchrow(
                spotify.user_id
            )
        ...
        playlists_stmt = await conn.prepare(query)
        playlists = await playlists_stmt.fetch(
            ignore, image_width, image_height, country, spotify.user_id
        )
        cities = [
            {"playlist": dict(playlist), "name": playlist["city"]}
            for playlist in playlists
        ]
        if with_countries:
            countries = [
                dict(c)
                for c in await conn.fetch("SELECT * FROM countries ORDER BY name")
            ]

    try:
        await assign_images(spotify, cities, "name", "city", image_width, image_height)
    except Exception as exc:
        logger.error("Error fetching images:")
        logger.exception(exc)
...
async def save_playlist(request):
    ...
    async with spotify.async_db_session(readonly=True) as conn:
        ...
        await spotify.user_playlist_replace_tracks(
            spotify.username, playlist.id, tracks
        )
        if image:
...
async def clone_playlist(request):
    ...
    description = await get_playlist_description(spotify, playlist_id)
    playlist = await spotify.user_playlist_create(
        spotify.username, name, description=description
    )
    ...
    async with spotify.async_db_session(readonly=True) as conn:
        if "genre" in args:
            ...
            playlists = await conn.fetch(
                "SELECT * FROM playlists WHERE genre = $1", genre
            )
        elif "genres" in args:
...
async def playlist_tuner(request, ws):
    ...
    while True:
        ...
        sender_task = loop.create_task(send_recommendations(data))
...
async def playback_controller(request, ws):
    ...
    async def handle_action(action):
        try:
            ...
            loop.create_task(send_playback())
    ...
    while True:
        ...
        loop.create_task(handle_action(action))
...
async def devices_watcher(request, ws, polling=30):
    ...
    asyncio.get_event_loop().create_task(polling_updater())
...
@app.websocket("/stats-collector")
async def stats_collector(request, ws):
    global STATS
    spotify = request["spotify"]
    ...
    while True:
        ...
        if any(p["measurement"] == SIDEBAR_ATTRIBUTE_METRIC for p in points):
            ...
            request.app.responses.pop(request_id, None)

        STATS += [fix_types(point) for point in points]
